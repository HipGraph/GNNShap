...
class SHAPUniqueSampler(BaseSampler):
    r"""`"This sampling algorithm is a modified version of KernelSHAP from:" 
        <https://github.com/slundberg/shap/blob/master/shap/explainers/_kernel.py>`. It makes sure
        each coalition is sampled only once.
        """
    ...
    def sample(self) -> Tuple[Tensor, Tensor]:
        ...
        It makes symmetric sampling: if [0, 0, 1] is sampled, [1, 1, 0] is also included.
        Therefore, the loops care only half of the samples.

        Returns:
            Tuple[Tensor, Tensor]:  mask_matrix (boolean) and weights
        """
        ...
        if num_full_subsets != num_subset_sizes:
            ...
            while samples_left > 0 and ind_set_pos < len(ind_set):
                ...
                new_sample = False
                if mask_tuple not in used_masks:  # temporarily disabled
                    new_sample = True
                    used_masks[mask_tuple] = nsamples_added
                    samples_left -= 1
                    addsample(mask, 1.0, nsamples_added)
                    nsamples_added += 1
                else:
                    kernel_weights[used_masks[mask_tuple]] += 1.0
                ...
                if samples_left > 0 and subset_size <= num_paired_subset_sizes:
                    ...
                    if new_sample:
                        ...
                        # we know the compliment sample is the next one after the
                        # original sample, so + 1
                        kernel_weights[used_masks[mask_tuple] + 1] += 1.0

            # normalize the kernel weights for the random samples to equal the weight left after
            # the fixed enumerated samples have been already counted
            weight_left = np.sum(weight_vector[num_full_subsets:])
            kernel_weights[nfixed_samples:] *= weight_left / \
                kernel_weights[nfixed_samples:].sum()
